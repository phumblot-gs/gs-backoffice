...
| Audit logging (CC7)     | `backoffice.*` events on every tool call (employee + agent), approval decision, evolution step, **PR merge + deploy**; multi-store durability (EVT + CloudWatch MCP-audit + bridge run-log + Paperclip `activity_log`). See **[audit-trail.md](./audit-trail.md)**. | Log retention over the observation window, immutability; audit-trail.md durability map |
...
1. **Segregation of duties** in change management: the go-live acts are now **recorded** in the audit trail — `backoffice.evolution.merged` (who merged) + `backoffice.deploy.completed` (see audit-trail.md). The remaining gap is **enforcement** of author ≠ approver: the agent opens PRs with a token tied to a human account, so the human currently self-approves/merges (admin bypass). Fix planned: a **dedicated GitHub identity** (GitHub App / machine user) for the agent so the human is a genuine independent approver. Branch protection on `main` already requires 1 review (admins not yet enforced).
